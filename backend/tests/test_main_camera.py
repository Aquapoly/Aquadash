...
from backend.app.services.cam_client import IMAGE_EXPIRE_TIME
import time

# Mocks
def mock_get_image_success(*args, **kwargs):
    new_image = b'plant image'
    cam_client._last_image = (new_image, time.time() + IMAGE_EXPIRE_TIME)
    return new_image


def mock_get_image_unexpired(*args, **kwargs):
    return cam_client._last_image[0]
...
def test_get_picture_success(client: TestClient, monkeypatch, set_last_image):
    ...
    monkeypatch.setattr(cam_client, "get_image", mock_get_image_success)
    ...
    assert cam_client._last_image[0] == b'plant image', "Should update image"
...
def test_get_picture_unexpired(client: TestClient, monkeypatch, set_last_image):
    ...
    monkeypatch.setattr(cam_client, "get_image", mock_get_image_unexpired)
...
def test_get_picture_error(client: TestClient, monkeypatch, set_last_image):
    ...
    monkeypatch.setattr(cam_client, "get_image", mock_get_image_error)
