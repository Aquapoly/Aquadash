...
export class StatusPageComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.sensorLastMeasurements = await Promise.all(this.sensors.map(async (sensor) => {
      const last_measurement: Measurement = await this.sensorService.getLastMeasurement(sensor.sensor_id);
  ...
  getLastMeasurement(sensor: Sensor): string {
    const last_measurement = this.sensorLastMeasurements.find((msr) => msr[0] === sensor.sensor_id)
    // Display no data if sensor has no measurements instead of undefined
    return last_measurement && last_measurement[1]? last_measurement[1]?.toString() : 'No data';
  }

  getSensorValidity(sensor: Sensor): string {
    const get_measurement = this.sensorLastMeasurements.find((msr) => msr[0] === sensor.sensor_id);
    ...
    (last_measurement < sensor.threshold_critically_low || 
      last_measurement > sensor.threshold_critically_high) ? "error" :
    (last_measurement < sensor.threshold_low || 
      last_measurement > sensor.threshold_high) ? "warning" :
    "success";
