...
export class SensorService {
    ...
    time_delta : WritableSignal<TimeDelta> = signal(this.rangeSelect[0].value);
    
    constructor(private httpClient: HttpClient) {}

    timeToPythonString(days: number, hours: number, minutes:number, seconds: number): string {
    ...
    getSensorMeasurementsDelta(sensor_id: number, time_delta: TimeDelta): Promise<Measurement[]> {
      const time_delta_str = time_delta.toString();
      return firstValueFrom(this.httpClient.get<Measurement[]>(`${this.serverUrl}/measurements/${sensor_id}?time_delta=${time_delta_str}`));
    }

    getSensorMeasurements(sensor_id: number, start_time: string, end_time: string): Promise<Measurement[]> {
      return firstValueFrom(this.httpClient.get<Measurement[]>(`${this.serverUrl}/measurements/${sensor_id}?start_time=${start_time}+end_time=${end_time}`));
    }

      
    getLastMeasurement(sensor_id: number): Promise<Measurement> {
      const observer =
        this.httpClient.get<Measurement>(`${this.serverUrl}/measurements/${sensor_id}/last`)
          .pipe(catchError(this.handleError<Measurement>(`error getting measurement for sensor id ${sensor_id}`)));
      return firstValueFrom(observer);
    }
      
    async getSensors(): Promise<Sensor[]> {
      const observer =
        this.httpClient.get<Sensor[]>(`${this.serverUrl}/sensors/${this.prototypeId}`)
          .pipe(catchError(this.handleError<Sensor[]>(`error getting sensors for prototype ${this.prototypeId}`)));
      return firstValueFrom(observer);
    }
